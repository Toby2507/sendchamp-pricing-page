...
const SingleProduct = ({ icon, id, title, sendDesc, recieveDesc, priceUnit, sendPrice, recievePrice, extraDetail }: SingleProductInterface) => {
  const setModalInfo = useSetRecoilState(productDetailsAtom);
  const showModal = () => setModalInfo({ show: true, id });
  return (
    <Card as={Flex} direction="column" border="1px solid" borderColor="gray.10" color="gray.800">
      ...
      {extraDetail && (
        <>
          ...
          <CardFooter as="h4" px="1.5rem" py={5} justify="center" color="primary.600" textAlign="center" textDecoration="underline" cursor="pointer" onClick={showModal}>More details</CardFooter>
        </>
      )}
    </Card>
  );
